...
const MenuItemList: React.FC<MenuItemListProps> = ({ logoutUser, setToggleMenu }) => {
  ...
  return (
    <>
      ...
      <MenuItem label="Active adds" hover onClick={() => {router.push(`/profile/${auth.currentUser ? auth.currentUser.uid : ''}`), setToggleMenu(false)}} icon={<MegaphoneIcon className="w-6 h-6" />} />
      <MenuItem label="Ended adds" onClick={() => {}} hover icon={<NewspaperIcon className="w-6 h-6" />} />
      <MenuItem label="Non-active adds" onClick={() => {}} hover icon={<ExclamationCircleIcon className="w-6 h-6" />} />
      <MenuItem label="Hidden adds" onClick={() => {}} hover icon={<EyeSlashIcon className="w-6 h-6" />} />
      <MenuItem label="Expired adds" onClick={() => {}} hover icon={<MinusCircleIcon className="w-6 h-6" />} />
      <MenuItem label="Renewable adds" onClick={() => {}} hover icon={<ArrowPathIcon className="w-6 h-6" />} />
      ...
      <MenuItem label="Saved Adds" onClick={() => {router.push('/myolx/saved/adds?tab=adds'), setToggleMenu(false)}} hover icon={<NewspaperIcon className="w-6 h-6" />} />
