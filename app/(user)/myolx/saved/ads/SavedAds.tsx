...
const SavedAds = () => {
  ...
  // check to delete ads which were deleted from "Products" but not from "Liked"
  useEffect(() => {
    if (products && allLikedAdds) {
      const likedRefs = allLikedAdds.map((liked) => liked.likedRef);
      const deletedRefs = likedRefs.filter(
        (likedRef) => !products.docs.some((product) => product.id === likedRef)
      );

      deletedRefs.forEach(async (deletedRef) => {
        try {
          await deleteDoc(doc(db, "users", userId, "savedAdds", deletedRef));
          toast({
            position: "top",
            status: "success",
            title: "An ad was removed from favorites",
          });
        } catch (error) {
          toast({
            position: "top",
            status: "error",
            title: "Ad deletion failed",
          });
        }
      });
    }
  }, [products, allLikedAdds, userId, toast]);
